...
From the above plot we can see that the normality assumption is most likely not being satisfied but we must use mathematical 
tests to prove this intuition. 
...
Because we have failed both the constant variance and the normality assumption we are going to perform a box-cox
transformation to look at and transform the data. 
...
The box cox plot tells us that we need to perform a log transform of the y variable in order to correct some of the violations of the original 
model
```{r}
chosen_model = lm(log(psalevel) ~ cancervolume + svi, data = prostate)
```
We must now go over the constant variance assumption and the normality assumption to see if this transformation has corrected the previous
issues. 
